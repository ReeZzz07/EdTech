...
import { runDiagnosisForImage, runDiagnosisForText } from "./aiService";
...
export async function runDiagnosisForProblemId(problemId: string) {
  const p = await prisma.problem.findUnique({
    where: { id: problemId },
    include: { subject: true, user: true, bankTask: true },
  });
  ...
  if (p.bankTaskId) {
    const taskBody = p.originalText?.trim() || p.bankTask?.body || "";
    const studentText = p.studentSolution?.trim() || "";
    if (!taskBody) {
      await prisma.problem.update({
        where: { id: p.id },
        data: { status: "error", completedAt: new Date() },
      });
      return { ok: false as const, reason: "no_task_text" as const };
    }
    try {
      ai = await runDiagnosisForText({
        subjectName: p.subject.name,
        taskText: taskBody,
        studentText,
      });
    } catch (e) {
      logger.error({ e, problemId }, "ai text");
      await prisma.problem.update({
        where: { id: p.id },
        data: { status: "error", completedAt: new Date() },
      });
