...
export function ProfileScreen() {
  ...
  return (
    <div className="p-4 pb-28">
      <h1 className="mb-4 text-xl font-bold text-tg-text">Профиль</h1>
      ...
      <div className="mb-6 flex items-center gap-4">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="text-sm text-tg-hint">Уровень {user?.level ?? 1}</p>
          {user?.isPremium ? (
            <p className="mt-1 flex items-center gap-1 text-sm font-medium text-amber-700">
              <Crown size={16} /> Premium
              {premiumUntil ? <span className="font-normal text-tg-hint">до {premiumUntil}</span> : null}
            </p>
          ) : (
            <button type="button" className="mt-1 text-sm text-tg-link" onClick={() => navigate("/premium")}>
              Подключить Premium
            </button>
          )}
        </div>
      ...
      <section className="mb-6 grid grid-cols-3 gap-2 text-center text-sm">
        <div className="rounded-xl border border-tg bg-tg-secondary p-3">
          <p className="text-tg-hint">EGC</p>
          <p className="font-semibold text-tg-text">{user?.coinBalance ?? 0}</p>
        </div>
        <div className="rounded-xl border border-tg bg-tg-secondary p-3">
          <p className="text-tg-hint">Задачи</p>
          <p className="font-semibold text-tg-text">{user?.totalProblemsSolved ?? 0}</p>
        </div>
        <div className="rounded-xl border border-tg bg-tg-secondary p-3">
          <p className="text-tg-hint">Серия</p>
          <p className="font-semibold text-tg-text">{user?.dailyStreak ?? 0}</p>
        </div>
      </section>
      ...
      <ul className="mb-6 space-y-2">
        {ACHIEVEMENT_DEFS.map((def) => {
          ...
          return (
            <li
              key={def.code}
              className={`rounded-xl border p-4 ${at ? "border-amber-400/45 bg-amber-500/12" : "border-tg bg-tg-secondary/80 opacity-80"}`}
            >
              <p className="font-medium text-tg-text">{def.title}</p>
              <p className="text-sm text-tg-hint">{def.description}</p>
              {at ? <p className="mt-1 text-xs text-tg-hint">Открыто {new Date(at).toLocaleDateString("ru-RU")}</p> : null}
            </li>
          );
        })}
      </ul>
