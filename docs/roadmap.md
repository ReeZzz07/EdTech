...
- [ ] **NFR из ТЗ** — lazy loading тяжёлых экранов (частично уже в `App.tsx`), WebP, retry безопасных GET в Axios (сделано), понятные ошибки; минимальный Service Worker для SPA shell _(prod: `public/sw.js`)_ — без полноценного offline API.
...
- [ ] **Синхронизация с ФИПИ** — отдельные джобы/скрипты, `fipiSpecKey`, наполнение справочников тем без смены схем `Problem`/`Diagnosis`.
- [ ] **Новые предметы** — только данные + конфиг skill map, без ломки клиента.
- [ ] **Peer help** — токенная оплата объяснений, UI `PeerHelpRequest` (см. ТЗ).
- [ ] **A11y & polish** — screen reader, контраст, визуальная регрессия, полный чеклист Mobile UX из ТЗ.
