...
- [ ] **Home & Profile** — карточки «Сегодня», прогресс, достижения, уровень; согласованность с `GET /me` и progress.
- [ ] **События аналитики** — `problem_solved`, `diagnosis_viewed`, `coins_earned` (по ТЗ).
...
- [ ] **ProgressScreen** — периоды, фильтр `subjectId`, Recharts, SkillMap из данных/конфига по предмету.
- [ ] **ShopScreen** — табы, карточки товаров, баланс, `POST /api/coins/spend`, история покупок, предложения дня (можно упростить в MVP).
- [ ] **PremiumScreen** — сравнение тарифов, копирайт, переход к оплате (реальная интеграция — этап 9).
- [ ] **UX лимитов** — дружелюбные сообщения при исчерпании дневного лимита, deeplink на Premium/магазин.
...
- [ ] **ClansScreen** — поиск, «мой клан», рекомендации, глобальный лидерборд, создание клана.
- [ ] **Чат клана (MVP+)** — базовая доставка сообщений (в ТЗ — «базовая интеграция»); при нехватке срока — эпик отложить и отметить в-roadmap.
